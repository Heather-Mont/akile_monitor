...
if [ "$#" -lt 3 ]; then
    echo "Usage: $0 <auth_secret> <url> <name> [proxy_enabled] [proxy_type] [proxy_address]"
    echo "Example 1 (without proxy): $0 your_secret wss://api.123.321 HK-Akile"
    echo "Example 2 (with proxy): $0 your_secret wss://api.123.321 HK-Akile true socks5 127.0.0.1:40000"
    exit 1
fi
...
# Create client configuration with proxy settings
